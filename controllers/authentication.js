...
exports.verificationCode = catchError(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError("user not exists please signUp or LogIn to continue", 404)
    );
  }
...
exports.logIn = catchError(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(
      new AppError("please enter your email and password to continue", 404)
    );
  }
  ...
  if (!user) {
    return next(
      new AppError(
        "user not exists please enter valide information or signUp to continue",
        404
      )
    );
  }
...
exports.restrictTo = (...roles) => {
  //this roles will be added in the route ['admin', 'teacher', 'parent'] where this function will be used
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError("you are not permitted to acces this route", 400)
      );
    }
  };
};
